...
const scores = SEARCH_FIELDS.map((field, i) => `CASE WHEN lower(${field})=$q THEN ${weights[i][0]}
  ...
  WHEN instr(lower(${field}),$q)>0 THEN ${weights[i][2]} ELSE 0 END`);
...
export class ParcelSearch {
  ...
  search(text: string, limit: number): ParcelSearchResult[] {
    ...
    const match = this.indexed && searchTrigrams(q, norm);
